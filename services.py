...
class DroneServer(Node):
    ...
    def __init__(self):
        ...
        self.MAX_TIME_OUT = 20.0

        
        self.srv = self.create_service(Move, 'move_forward', self.move_forward_callback)
        self.srv = self.create_service(Move, 'rotate_clockwise', self.rotate_clockwise_callback)
        self.srv = self.create_service(Move, 'arotate_clockwise', self.arotate_clockwise_callback)
        ...
        self.srv = self.create_service(Move, 'go', self.go_callback)
        self.srv = self.create_service(Move, 'curve', self.curve_callback)
    ...
    def curve_callback(self, request, response):
        global drone_response
        self.get_logger().info('Incoming request: Curve: x1=%d, y1=%d, z1=%d, x2=%d, y2=%d, z2=%d, speed=%d' % (
        request.x1, request.y1, request.z1, request.x2, request.y2, request.z2, request.speed))

        command = f"curve {request.x1} {request.y1} {request.z1} {request.x2} {request.y2} {request.z2} {request.speed}"
        print(command)
        self.send_command("command")
        time.sleep(1)
        self.send_command(command)
    ...
    def go_callback(self, request, response):
        global drone_response
        self.get_logger().info('Incoming request: go: x=%d, y=%d, z=%d, speed=%d' % (request.x, request.y, request.z, request.speed))

        command = f"go {request.x} {request.y} {request.z} {request.speed}"
        print(command)
        self.send_command("command")
        time.sleep(1)
        self.send_command(command)
    ...
    def rotate_clockwise_callback(self, request, response):
        global drone_response
        self.get_logger().info('Incoming request: Rotate Clockwise: %d' % (request.distance))
        command = "cw %d" % request.distance
        print(command)
        self.send_command("command")
        time.sleep(1)
        self.send_command(command)
    ...
    def arotate_clockwise_callback(self, request, response):
        global drone_response
        self.get_logger().info('Incoming request: aRotate Clockwise: %d' % (request.distance))
        command = "ccw %d" % request.distance
        print(command)
        self.send_command("command")
        time.sleep(1)
        self.send_command(command)
